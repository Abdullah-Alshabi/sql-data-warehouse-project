...
Script Purpose:
    ...
    - Prints messages for each section and step.
    - Prints the duration of each step and the total duration of loading Silver.
	...
	BEGIN TRY
		...
		SELECT 
			...
			CASE 
				...
				ELSE 'n/a'
			END cst_material_status, -- Normlize marital status values to readable format
			CASE 
				WHEN UPPER(TRIM(cst_gndr)) = 'F' THEN 'Female'
				WHEN UPPER(TRIM(cst_gndr)) = 'M' THEN 'Male'
				ELSE 'n/a'
			END cst_gndr, -- Normlize gender values to readable format
			cst_create_date
		FROM (
			SELECT 
				*,
		...
		SELECT
			prd_id,
			REPLACE(SUBSTRING(prd_key, 1, 5), '-', '_') AS cat_id, -- Exstract category ID
		...
		SELECT 
			...
			CASE 
				...
				ELSE TRIM(cntry)
			END AS cntry -- Normlize and handle missing or blank country codes
